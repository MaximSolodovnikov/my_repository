...
function get_menu() {
	...
	$select_sql = sprintf("SELECT * FROM `menu` WHERE `menu`.`title_url` != 'registration' AND `menu`.`title_url` != 'search' AND `menu`.`title_url` != 'forgot' AND `menu`.`title_url` != 'books' AND `menu`.`title_url` != 'users'");
...
function get_products_by_cat($table, $title) {
	
	db_connect();
	
	$select_sql = sprintf("SELECT * FROM $table WHERE $table.title_url = '%s'",
							mysql_real_escape_string($title));
	$result = mysql_query($select_sql);
	$row = mysql_fetch_array($result);
...
/*function get_products_by_cat($title) {
	
	db_connect();
	
	$select_sql = sprintf("SELECT * FROM products WHERE products.cat = '$title'");
	$result = mysql_query($select_sql);
	$result = db_result_to_array($result);
	return $result;
}*/
...
function leave_comment() {
	...
	$insert_sql = sprintf("INSERT INTO comments (author, text, date, time) VALUES ()")
}
